...
add_filter( 'gppa_hydrated_field_123_4', function( $return ) {
	$func = function( $merge_tag_match_value, $merge_tag, $form, $field_id, $entry_values ) use ( &$func ) {
		// Update "5" to the ID of the field targeted by the live merge tag.
		remove_filter( "gppa_live_merge_tag_value_{$form['id']}_5", $func );
		return $merge_tag_match_value . ' FILTERED!';
	};
	add_filter( 'gppa_live_merge_tag_value', $func, 10, 5 );
	return $return;
} );
